...
fn changelog_destinations(version: &str) -> &'static [(&'static str, &'static str)] {
    match version {
        "0.1.27" | "0.1.26" | "0.1.23" => &[("/diagnostics", "Diagnostics")],
...
<div class="toolbar"><span id="live" class="live">Connecting</span><div class="actions"><button id="refresh" type="button">Refresh</button><button id="copy" type="button">Copy snapshot</button></div></div><section class="grid">
...
(()=>{{const bridge=window.RustDLDiagnostics,$=selector=>document.querySelector(selector);let latest=null,busy=false;const clamp=value=>Math.max(0,Math.min(100,value));const bytes=value=>{{if(value<0)return'Unavailable';const units=['B','KB','MB','GB','TB'];let index=0;while(value>=1024&&index<units.length-1){{value/=1024;index++}}return value.toFixed(index>2?1:0)+' '+units[index]}};const duration=seconds=>{{if(seconds<0)return'Unavailable';const days=Math.floor(seconds/86400),hours=Math.floor(seconds%86400/3600),minutes=Math.floor(seconds%3600/60);return days?days+'d '+hours+'h':hours?hours+'h '+minutes+'m':minutes+'m'}};const thermalNames=['None','Light','Moderate','Severe','Critical','Emergency','Shutdown'];const setBar=(id,value)=>$(id).style.width=clamp(value)+'%';
const render=data=>{{latest=data;const memoryUsed=data.memoryTotalBytes>0?100*(1-data.memoryAvailableBytes/data.memoryTotalBytes):0,storageUsed=data.storageTotalBytes>0?100*(1-data.storageAvailableBytes/data.storageTotalBytes):0,cpuUsed=data.processors>0?100*data.load1/data.processors:0,thermalTemperature=data.batteryTemperatureC,thermalStatusValue=data.thermalStatus,thermalStatus=thermalStatusValue>=0?(thermalNames[thermalStatusValue]||'Status '+thermalStatusValue):'Status unavailable';$('#battery').textContent=data.batteryLevel>=0?data.batteryLevel+'%':'Unavailable';$('#battery-sub').textContent=(data.batteryStatus||'Unknown')+' · '+(data.batteryTemperatureC>=0?data.batteryTemperatureC.toFixed(1)+' °C':'temperature unavailable');setBar('#battery-bar',data.batteryLevel);$('#thermal').textContent=thermalTemperature>=0?thermalTemperature.toFixed(1)+' °C':thermalStatus;$('#thermal-sub').textContent=(thermalTemperature>=0?'Battery sensor · ':'')+thermalStatus;setBar('#thermal-bar',thermalTemperature>=0?(thermalTemperature-20)/30*100:thermalStatusValue<0?0:thermalStatusValue/6*100);$('#cpu').textContent=data.load1>=0?data.load1.toFixed(2):'Unavailable';$('#cpu-sub').textContent=data.load1.toFixed(2)+' / '+data.load5.toFixed(2)+' / '+data.load15.toFixed(2)+' · '+data.processors+' cores';setBar('#cpu-bar',cpuUsed);$('#memory').textContent=bytes(data.memoryAvailableBytes);$('#memory-sub').textContent=memoryUsed.toFixed(0)+'% used of '+bytes(data.memoryTotalBytes);setBar('#memory-bar',memoryUsed);$('#storage').textContent=bytes(data.storageAvailableBytes);$('#storage-sub').textContent=storageUsed.toFixed(0)+'% used of '+bytes(data.storageTotalBytes);setBar('#storage-bar',storageUsed);$('#uptime').textContent=duration(data.uptimeSeconds);$('#app-pid').textContent=data.rustdlPid>0?'PID '+data.rustdlPid:'Offline';$('#updated').textContent='Updated '+new Date(data.timestamp).toLocaleTimeString()}};
const load=()=>{{if(busy)return;busy=true;if(!bridge){{$('#live').textContent='Open inside RustDL';$('#live').classList.add('offline');busy=false;return}}try{{const response=JSON.parse(bridge.diagnostics());if(!response.ok||!response.data)throw new Error(response.detail);render(response.data);$('#live').textContent='Live · every 3 seconds';$('#live').classList.remove('offline')}}catch(error){{$('#live').textContent='Diagnostics unavailable';$('#live').classList.add('offline')}}finally{{busy=false}}}};$('#refresh').addEventListener('click',load);$('#copy').addEventListener('click',async event=>{{if(!latest)return;try{{await navigator.clipboard.writeText(JSON.stringify(latest,null,2));event.currentTarget.textContent='Copied';setTimeout(()=>event.currentTarget.textContent='Copy snapshot',900)}}catch(_error){{}}}});document.addEventListener('visibilitychange',()=>{{if(!document.hidden)load()}});load();setInterval(()=>{{if(!document.hidden)load()}},3000)}})();
</script>{}</body></html>"#,
...
mod tests {
    ...
    fn changelog_covers_every_version_and_marks_the_current_release() {
        ...
        assert_eq!(html.matches(r#"class="release-actions""#).count(), 26);
